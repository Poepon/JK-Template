...
namespace JK.Authentication.TwoFactor.Google
{
    public class GoogleAuthenticatorProvider : AbpServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
    {        
        ...
        public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
        {
            ...
            var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode(GoogleAuthenticatorConst.Issuer, user.UserName, user.GoogleAuthenticatorKey, 300, 300);
